...
void UHeroGameplayAbility_PickUpStones::CollectStones()
{
	...
	UKismetSystemLibrary::BoxTraceMultiForObjects(
		...
		GetHeroCharacterFromActorInfo()->GetActorLocation() + -GetHeroCharacterFromActorInfo()->GetActorUpVector() *
		BoxTraceDistance,
		TraceBoxSize / 2.f,
...
void UHeroGameplayAbility_PickUpStones::ComsumeStones()
{
	if (CollectedStones.IsEmpty())
	{
		CancelAbility(GetCurrentAbilitySpecHandle(), GetCurrentActorInfo()
	...
	for (AWarriorStoneBase* CollectedStone : CollectedStones)
	{
		if (CollectedStone)
		{
			CollectedStone->Comsume(GetWarriorsAbilitySystemComponentFromActorInfo(), GetAbilityLevel());
		}
	}
}
