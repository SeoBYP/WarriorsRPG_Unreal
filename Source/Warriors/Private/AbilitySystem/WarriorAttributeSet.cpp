...
void UWarriorAttributeSet::PostGameplayEffectExecute(const struct FGameplayEffectModCallbackData& Data)
{
	...
	checkf(PawnUIComponent, TEXT(" didn't implement from IPawnUIInterface %s"),
	...
	if (Data.EvaluatedData.Attribute == GetCurrentRageAttribute())
	{
		...
		if (GetCurrentRage() == GetMaxRage())
		{
			UWarriorFunctionLibrary::AddGameplayTagToActorIfNone(
				...
				WarriorsGameplayTags::Player_Status_Rage_Full);
		}
		else if (GetCurrentRage() == 0.0f)
		{
			UWarriorFunctionLibrary::AddGameplayTagToActorIfNone(
				Data.Target.GetAvatarActor(),
				WarriorsGameplayTags::Player_Status_Rage_None);
		}
		else
		{
